...
{
    //Hook for OnPlayerTalentsReset Event
            ...
            if (slot < INVENTORY_SLOT_BAG_END)
            {
                ...
                if (slot < EQUIPMENT_SLOT_END)
                {
                    ...
                    if (GetUInt32Value(PLAYER_VISIBLE_ITEM_1_0 + (slot * MAX_VISIBLE_ITEM_OFFSET)) != pItem->GetEntry())
                        CharacterDatabase.PExecute("DELETE FROM _ark_characters_transmog WHERE guid = %u AND slot = %u", pItem->GetOwnerGuid().GetCounter(), slot);
                }
            }
...
{
    //QueryResult *result = CharacterDatabase.PQuery("SELECT spell,spec FROM character_talent WHERE guid = '%u'",GetGUIDLow());
    if (result)
    {
        do
        {
