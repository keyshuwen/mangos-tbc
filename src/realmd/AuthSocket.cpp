...
bool AuthSocket::_HandleLogonProof()
{
    ...
    if (!srp.Proof(lp.M1, 20))
    {
        ...
        LoginDatabase.PExecute("UPDATE account SET sessionkey = '%s', last_ip = '%s', last_login = NOW(), locale = '%u', os = '%s', failed_logins = 0 WHERE username = '%s'", K_hex, m_address.c_str(), GetLocaleByName(_localizationName), m_os.c_str(), _safelogin.c_str());
